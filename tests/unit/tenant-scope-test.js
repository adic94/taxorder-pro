...
const ZNANE_BEZPIECZNE = {
  ...
  "handover_protocols|UPDATE handover_protocols SET typ=?, data=?, osoba_wydajaca=?, osoba_odbierajaca=?, stan_licznika=?, stan_paliwa=?, wyposazenie=?, uszkodzenia_opis=?, uwagi=?, podpis_wydajacy=?, podpis_odbierajacy=? WHERE id=?": "[PRE-CHECK] poprzedzone SELECT * FROM handover_protocols WHERE id=? AND company_id=?",
