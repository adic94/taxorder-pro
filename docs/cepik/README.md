...
### 0.2 Napraw rejestr dostępu — bez tego wniosek jest nieprawdziwy

`logAudit()` (`worker/index.js:6957`) jest zaimplementowana, ale **nie jest wywoływana
z żadnego miejsca w kodzie**. Tabela `audit_logs` jest pusta.

To jest dokładnie **warunek ustawowy nr 1** dla teletransmisji: „urządzenia umożliwiające
odnotowanie w systemie, kto, kiedy, w jakim celu oraz jakie dane uzyskał". Wydanie decyzji
poprzedza **audyt potwierdzający wykazaną specyfikację** — deklaracja niezgodna ze stanem
faktycznym zostanie tam ujawniona.

Do zrobienia przed wysyłką:
1. wywołać `logAudit()` w `handleCepikPojazdy` (`worker/index.js:3803`) i pozostałych
   ścieżkach CEPiK, z `action`, `details` (cel i zakres) oraz `entity_id` pojazdu;
2. ograniczyć zapytania do pojazdów z własnej ewidencji — dziś `handleCepikPojazdy`
   przyjmuje dowolny numer rejestracyjny;
3. potwierdzić, że wpisy faktycznie powstają (`GET /api/audit-log`).
...
| Nr | Załącznik | Skąd |
|---|---|---|
| 1 | Opis systemu teleinformatycznego | `zalacznik-1-opis-systemu.md` — uzupełnij sekcje `⚠` |
...
> jest już w kodzie projektu, w komentarzu przy `handleCepikKierowcaV2`
> (`worker/index.js:3864`).
