...
| `dr-ocr-batch-cloudrun.js` | **Właściwa naprawa klastra "zero z CF/Groq" — woła Cloud Run (PaddleOCR) BEZPOŚREDNIO, z pominięciem limitu 8s Workera.** Znalezisko 25.08: dokumenty typu `WE6LR8x`/`WE6LT5x` (klaster Toyot Hilux GR) mają treść strony PDF narysowaną w orientacji pionowej mimo fizycznie poziomego dowodu — `page.rotate`=0, obrót zaszyty w macierzy rysowania obrazu, nie we fladze PDF, więc żaden model dostający nieobrócony render nie miał szans. `ocr-service/extractors/paddle_fields.py` (PaddleOCR lang=pl, `use_doc_orientation_classify=True`) prostuje to automatycznie — zweryfikowane na WE6LR80: z zera pól (dwie pełne rundy CF+Groq) do 8 poprawnych pól, zgodnych z ręczną inspekcją wizualną obróconego dokumentu. **Dlaczego osobne narzędzie, nie ponowne włączenie Próby 0 w Workerze:** bez akceleratora CPU oneDNN (pada twardym crashem na Cloud Run — SIGFPE, dwie różne awarie na dwóch wersjach paddlepaddle, patrz komentarz w `ocr-service/requirements.txt`) jeden dokument to ~30–80s na 4 vCPU — daleko od limitu 8s, ale bez znaczenia dla wsadu uruchamianego z tego narzędzia. Wymaga `OCR_PYTHON_SECRET`/`OCR_PYTHON_URL` w `.env` |
