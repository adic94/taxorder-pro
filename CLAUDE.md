...
| 2026-09-01 | **Skrzynka dokumentów naprawiona i pierwszy raz realnie użyteczna — PR #71-74 scalone, `migration_v54` zaaplikowana, produkcja zweryfikowana po każdym deployu.** `documents.nr_rej` było `NOT NULL` od `schema_v1` — orphan-upload (dokument bez jeszcze dopasowanego pojazdu, dokładnie scenariusz „wrzucam plik, system ma znaleźć pojazd") zawsze padał na zapisie; `handleFmIngest` (agent Node.js `tools/folder-watcher/`) robił OCR i WYRZUCAŁ oryginalny plik, nic nie zapisując do R2/`documents`. Oba potwierdzone na produkcyjnym D1 przed naprawą, nie założone: `documents` miało 1 wiersz, `folder_monitor_queue` 0 — skrzynka nigdy nie zadziałała na tyle, żeby ktoś jej realnie użył. `migration_v54_documents_inbox.sql` (rebuild, SQLite nie zdejmie `NOT NULL` przez `ALTER`) — `nr_rej` nullable + nowa kolumna `ocr_fields` (JSON pełnego wyniku OCR). Doszedł eksport **„Eksportuj dane DR → XLSX"** na stronie Dokumenty flotowe (`DocumentsModule.exportDrXlsx()`) — lżejsza, chmurowa wersja `tools/dr-excel.js`: arkusze Pojazdy DR / Pokrycie / **Konflikty** (wykrywa rozbieżne odczyty OCR między wieloma skanami tego samego pojazdu — ten sam wzorzec ostrzeżenia co w lokalnym narzędziu, nie kopiowany na ślepo). Katalog `modules/dr-fields.js` (dotąd wyłącznie w `tools/`) dopisany do `index.html` zamiast piątej kopii listy pól.

  **Recenzja zastępująca Codex** (bot miał wyczerpany limit na wszystkich czterech PR-ach) znalazła i naprawiła realne błędy przed mergem: `handleExternalAccess` odcinał rolę `superadmin` od cross-tenant (regresja we własnym guardzie z PR #73, sprzeczna z przedistniejącym kodem); `exportDrXlsx()` czytał `window.DrFields` jak tablicę zamiast `window.DrFields.POLA` (rzuciłoby błędem przy pierwszych realnych danych) i ucinał eksport limitem 500 dokumentów wszystkich typów łącznie; konfiguracja E2E z samym kontem nie-admina crashowałaby testy na starcie (`global-setup.js` wychodził z funkcji przed zalogowaniem tego konta).

  **Kolejność merge → migracja zabezpieczona w obie strony**, nie tylko udokumentowana: `POST /api/docs/upload` i `GET /api/docs` (oba aktywnie używane) dostały fallback na starą wersję zapytania przy błędzie „no such column" — scalenie PR-a przed migracją nie zepsułoby działających endpointów.

  **Wykonane pod nieobecność właściciela, na jego wyraźne polecenie** ("wykonaj wszystkie działania, będę nieobecny"): scalone PR #71 (docs, zero ryzyka), #72 (testy E2E, zero zmian w kodzie produkcyjnym), #73 i #74 (`worker/index.js` — realny deploy). Po KAŻDYM mergu dotykającym Workera: `deploy-worker.yml` zweryfikowany jako `conclusion:success`, a `health-check.yml` odpalony ręcznie (`workflow_dispatch`) i sprawdzony jako zielony — nie samo „CI przeszło", tylko potwierdzenie żywej odpowiedzi produkcji. `migration_v54` zaaplikowana krokami z weryfikacją między nimi (jak `v53`, nie jednym plikiem przez `wrangler d1 execute` — ten kanał tego nie ma): `CREATE` → `INSERT` → porównanie liczby wierszy (1=1) → `DROP`+`RENAME` → odczyt `sqlite_master` i ponowne liczenie wierszy. Zero utraconych danych, zero osieroconej tabeli tymczasowej.

  **Co zostaje mimo pełnej autonomii tej rundy — bo to nie brak zgody, tylko brak narzędzia:** 402 na `/api/co2-report`/`/api/esg-targets` (podejrzenie: cache KV `mod_access_v1:*` albo stan sekretu `MODULE_ENFORCEMENT`) — ta sesja nie ma odczytu pojedynczych kluczy KV ani sekretów Workera, więc diagnoza kończy się na „to nie ten PR", nie na naprawie. Zgłoszone komentarzem na każdym z trzech PR-ów, które na to trafiły. | PR #71–74 |
| 2026-09-01 | **Realizacja rekomendacji z audytu funkcjonalności — instrukcja wdrożenia od zera była AKTYWNIE ZEPSUTA, nie tylko nieaktualna.** Rekomendacja audytu „usuń martwy `worker/schema.sql` (albo oznacz jako archiwalny)" doprowadziła do głębszego znaleziska przy sprawdzaniu, kto się do niego odwołuje: `CLOUDFLARE_DEPLOY.md` (Krok 4, aktywna instrukcja „wdróż od zera") i nagłówek `worker/index.js` KAZAŁY uruchomić SAM `schema.sql` — plik z pierwszej wersji projektu, 7 tabel. Ktoś idący tą instrukcją dostałby bazę bez firm, KSeF, tachografów, floty rezerwacji i dziesiątek innych tabel; Worker zacząłby padać na `no such table` przy pierwszym użyciu czegokolwiek poza podstawowym CRUD-em pojazdów — cichy dług od momentu wprowadzenia `schema_v2.sql`, nigdy nieużyty w praktyce, bo jedyne realne wdrożenie (produkcja) rosło migracja po migracji, nie od zera. Naprawione: `CLOUDFLARE_DEPLOY.md` Krok 4 opisuje teraz pełną pętlę po `schema_v*.sql` (bez `_ROLLBACK`, `sort -V`) + `migration_v*.sql`; nagłówek `worker/index.js` wskazuje na tę procedurę zamiast duplikować ją w komentarzu; `worker/schema.sql` dostał na górze blok „PRZESTARZAŁE — NIE UŻYWAJ", z wyliczeniem dokładnie tego, czego zabraknie.

  **Przy tej samej rekomendacji, druga część audytu — trzy martwe tabele od strony przeciwnej.** Wcześniejsze „sześć martwych tabel" (27.08, migracja v51) to tabele, których HANDLERY UŻYWAŁY, ale kolumny nie pasowały. Te trzy to odwrotny przypadek: schemat je tworzy (`schema_v35`: `approval_requests`, `driver_behavior_events`; `schema_v46`: `fixed_asset_depreciation`), a **żaden handler w `worker/index.js` nigdy się do nich nie odwołuje** — zweryfikowane `grep` po nazwie tabeli i po prawdopodobnych nazwach handlera/pól (zero trafień, nie licząc komentarza dokumentującego schemat w `modules/fixed-assets.js`). Realny przepływ zatwierdzeń idzie przez `approvals`/`approval_levels` (inne nazwy, inny handler — `handleApprovals`, `handleApprovalLevels`), a amortyzacja środków trwałych jest liczona wprost w kolumnach `fixed_assets` (`current_book_value`, `depreciation_method`, `depreciation_rate`) bez osobnej tabeli historii. **Nie usunięto ich i nie dopisano do żadnej bramki** — w przeciwieństwie do `ZNANE_BRAKI_TABEL` w `tests/unit/worker-columns-test.js` (worker odwołuje się do tabeli, której schemat NIE tworzy), to jest przypadek odwrotny, którego ten gate nie mierzy i do którego nie pasuje żaden istniejący format listy; usunięcie tabeli ze schematu jest nieodwracalne bez kopii, więc to zostaje jako świadomie udokumentowane znalezisko, nie wykonana naprawa. **Zastrzeżenie:** liczba wierszy na produkcji NIE została zweryfikowana w tej rundzie — zapytanie do produkcyjnego D1 zwróciło `403 not authorized` (ta sesja straciła dostęp, który miała wcześniej tego samego dnia); wniosek „martwe" opiera się wyłącznie na analizie statycznej kodu (zero odwołań w `worker/index.js`), nie na potwierdzeniu `COUNT(*)=0`. | — |
