...
function setV(id,k,val) {
  const v=vehs.find(x=>x.id===id);
  if(v){
    v[k]=val;

    if(window.TaxOrderFleetCloud && typeof window.TaxOrderFleetCloud.saveVehicle === "function"){
  ...
  try{
    ...
    for(let gi=0;gi<groups.length;gi++){
      ...
      try{
        ...
        tfp(fm,'numer zaĹ‚Ä…cznika',`${attNo}/${groups.length}`,fa,12);
...
function doLogin(){
  ...
  if(!email||!pass){showLoginErr('Wpisz e-mail i hasĹ‚o');return;}
  const u=users.find(x=>x.email.toLowerCase()===email&&x.passwordHash===btoa(pass)&&x.active);
  if(!u){showLoginErr('BĹ‚Ä™dny e-mail lub hasĹ‚o');return;}
  currentUser=u;
...
// Udostępnienie danych floty dla modułów zewnętrznych, np. migracji Supabase
