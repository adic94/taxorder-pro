...
window.TaxOrderSetCompanies = function(nextCompanies){
  if(!nextCompanies || !Object.keys(nextCompanies).length) return;
  COMPANIES = nextCompanies;

  if(!COMPANIES[currentCompanyId]){
    currentCompanyId = Object.keys(COMPANIES)[0];
    localStorage.setItem('dt1_current_company', currentCompanyId);
  }

  const sel = document.getElementById('company-selector');
  if(sel){
    sel.innerHTML = Object.values(COMPANIES).map(c => `<option value="${c.id}">${c.shortName}</option>`).join('');
...
function renderCompanyOverview(){
  if(window.TaxOrderCompanyManager){ window.TaxOrderCompanyManager.renderCompanyAdmin(); return; }
...
window.addEventListener('load', async () => {
  if(window.TaxOrderCompanyManager){ await window.TaxOrderCompanyManager.loadCompanies(); }
