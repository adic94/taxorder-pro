...
async function doLogin(){
  ...
  if(window.TaxOrderAuth && typeof window.TaxOrderAuth.login === 'function'){
    ...
    if(!authResult.ok){
      showLoginErr('BĹ‚Ä…d logowania Supabase: ' + (authResult.error?.message || 'brak sesji'));
      return;
    }
...
async function resetPasswordFlow(){
  ...
  if(!window.TaxOrderAuth || typeof window.TaxOrderAuth.resetPassword !== 'function'){
    showLoginErr('Reset hasĹ‚a nie jest jeszcze dostÄ™pny.');
    return;
  }
  ...
  if(!result.ok){
    showLoginErr('Nie udaĹ‚o siÄ™ wysĹ‚aÄ‡ linku resetujÄ…cego: ' + (result.error?.message || 'bĹ‚Ä…d'));
    return;
  }

  showLoginErr('WysĹ‚ano link do ustawienia nowego hasĹ‚a na adres: ' + email);
}

async function showNewPasswordModal(){
  const newPassword = prompt('Wpisz nowe hasĹ‚o:');

  if(!newPassword || newPassword.length < 6){
    alert('HasĹ‚o musi mieÄ‡ minimum 6 znakĂłw.');
    return;
  }
  ...
  if(!result.ok){
    alert('Nie udaĹ‚o siÄ™ zmieniÄ‡ hasĹ‚a: ' + (result.error?.message || 'bĹ‚Ä…d'));
    return;
  }

  alert('HasĹ‚o zostaĹ‚o zmienione. MoĹĽesz siÄ™ teraz zalogowaÄ‡ nowym hasĹ‚em.');
