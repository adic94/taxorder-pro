...
window.TaxOrderBudget = (function () {
  const STORE_KEY = 'taxorder-budget-plan';

  function _yr() { return +(document.getElementById('budp-year')?.value || new Date().getFullYear()); }
  function _co() { return document.getElementById('budp-company')?.value || ''; }
  function _fmt(n) { return (+n || 0).toLocaleString('pl-PL', { maximumFractionDigits: 0 }); }
  function _fmtZ(n) { return _fmt(n) + ' zł'; }

  function _getPlan(yr) {
    try { return JSON.parse(localStorage.getItem(STORE_KEY + '-' + yr)) || {}; }
    catch { return {}; }
  }
  function _savePlanData(yr, plan) {
    localStorage.setItem(STORE_KEY + '-' + yr, JSON.stringify(plan));
  }
  ...
  function _computeRow(v, pfx) {
    ...
    const tco      = fuel + service + insur + fines + tax;
    return { fuel, service, insur, fines, tax, tco };
  }
  ...
  function render() {
    ...
    if (kpiEl) {
      ...
      kpiEl.innerHTML = [
        ...
        ['Śr. TCO / pojazd',  _fmtZ(avgTco),  top ? 'Max: ' + esc(top.v.nrRej) : '', 'var(--text)'],
      ].map(([l,v,s,c]) => `<div class="stat"><div class="stat-label">${l}</div><div class="stat-val" style="color:${c};font-size:20px">${v}</div><div class="stat-sub">${s}</div></div>`).join('');
    }

    // Budget plan vs execution
      ...
      planEl.innerHTML = cats.map(c => {
        ...
        return `<div style="margin-bottom:12px">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
            ...
            ${b ? `<span style="font-size:11px;${over?'color:var(--red);font-weight:700':'color:var(--green)'}">/ ${_fmtZ(b)} (${pct.toFixed(0)}%)</span>` : `<input type="number" class="fi" data-bkey="${c.key}" placeholder="Plan zł" style="width:90px;height:24px;font-size:11px;margin:0;text-align:right">`}
          </div>
        ...
        </div>` : '<div style="font-size:11px;color:var(--text3);margin-top:8px">Wpisz wartości budżetu w polach powyżej i kliknij <strong>Zapisz plan</strong>.</div>');
    }
      ...
      tcoEl.innerHTML = `<table style="font-size:12px">
        ...
        <tbody>${top.map((r,i) => {
          ...
          return `<tr>
            ...
            <td><strong style="font-family:var(--mono)">${esc(r.v.nrRej)}</strong></td>
            <td style="color:var(--text2)">${esc(r.v.marka)} ${esc(r.v.model)}</td>
  ...
  function exportExcel() {
    ...
    const rows = vehs.map(v => {
      ...
      return ['Nr rej.','Marka','Model','Rok','Paliwo','Serwis','Ubezp.','DT-1','Mandaty','TCO'].reduce((obj, h, i) => {
        const vals = [v.nrRej, v.marka||'', v.model||'', v.rok||'', r.fuel, r.service, r.insur, r.tax, r.fines, r.tco];
        obj[h] = vals[i]; return obj;
      }, {});
    }).sort((a,b) => b.TCO - a.TCO);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), 'TCO ' + yr);
    XLSX.writeFile(wb, 'budzet-tco-' + yr + '.xlsx');
