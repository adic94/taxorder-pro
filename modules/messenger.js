(function () {
  ...
  async function api(path, opts={}) {
    ...
    const r = await fetch(`${API()}/api/messages${path}${sep}company=${co()}`, { headers:{'Content-Type':'application/json','Authorization':`Bearer ${localStorage.getItem('cf_token')}`}, ...opts });
    return r.json();
  }
